...
nested_compound_test() ->
    ...
    ?assertEqual(decode:decode_nbt(Data), [{tag_compound, "hello world", [{tag_compound, "hello world", [{tag_byte, "name", 1}]}, {tag_byte, "name", 1}]}]).

big_test() ->
    Data = get_data(),
    Result = bigtest_decoded:get_bigtest_decoded(),
    ?assertEqual(Result, decode:decode_nbt(Data)).
